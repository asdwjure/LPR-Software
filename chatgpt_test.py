...
frame = cv2.imread('images/slo3.jpg')

# Convert the frame to grayscale for easier processing
gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)

cv2.imshow('Original slika', gray)
cv2.waitKey(0)
cv2.destroyAllWindows()

# Detect license plates in the frame
plates = plate_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=4, minSize=(50, 30))
...
for (x, y, w, h) in plates:
    ...
    cv2.imshow('Sive tablice', plate_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
